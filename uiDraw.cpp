 ...
 *   INPUT  pos     The location of the projectile
 *          age     The age in seconds. The younger, the brighter
 *************************************************************************/
void ogstream::drawProjectile(const Position& pos, double age)
{
    const double tailLength = 10.0;

    // Calculate color based on age. Starts bright and fades to black.
    GLfloat colorIntensity = 0.0 + (age / tailLength);  // Fades from 1.0 (white) to 0.0 (black)
    ...
    drawRectangle(posBegin, posEnd, colorIntensity, colorIntensity, colorIntensity);
}

void ogstream::drawSmallProjectile(const Position& pos, double age)
{
    Position posBegin;
    Position posEnd;
    posBegin.setPixelsX(pos.getPixelsX() - 0.5);
    posBegin.setPixelsY(pos.getPixelsY() - 0.5);
    posEnd.setPixelsX(pos.getPixelsX() + 0.5);
    posEnd.setPixelsY(pos.getPixelsY() + 0.5);

    drawRectangle(posBegin, posEnd, 1, 0, 0);
}
